   ...
   BEGIN
      IF (@loginame like '[0-9]%')	-- is numeric
	     BEGIN
		    ...
		    FROM 
			   sys.dm_exec_requests er CROSS APPLY 
			   sys.dm_exec_sql_text(er.sql_handle) st CROSS APPLY 
			   sys.dm_exec_text_query_plan(er.plan_handle, er.statement_start_offset,
